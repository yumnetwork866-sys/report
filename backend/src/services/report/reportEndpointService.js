...
const { serializeBookingWithActual } = require('../bookingVideoPerformanceService');
const {
  loadMonthlyShopVideoRevenue,
  loadVideoDailyRevenue,
} = require('../channelReportRevenueService');
...
const channelReportBaseSql = `
  ...
  report_videos AS MATERIALIZED (
    SELECT
      ...
      video.video_url,
      video.thumbnail_url,
      COALESCE(video.status, 'active') AS status,
      video.published_at,
      video.views,
      video.likes,
      video.comments,
...
const getChannelReport = async (req, res) => {
  try {
    ...
    const [aggregateRows, teamRows, videoRows, productRows] = await Promise.all([
      reportRepository.query(`${channelReportBaseSql}
      ...
      reportRepository.query(`${channelReportBaseSql}
        ...
        , member_metrics AS (
          SELECT
            ...
            COUNT(video.id) FILTER (WHERE video.status = 'unavailable')::bigint AS unavailable_videos,
    ...
    for (const row of teamRows) {
      ...
      if (row.user_id) {
        team.members.push({
          ...
          name: row.member_name,
          videos: number(row.videos),
          unavailableVideos: number(row.unavailable_videos),
          views: number(row.views),
          revenue: number(row.revenue),
...
const getReportById = async (req, res) => {
  try {
    const report = await reportRepository.findReportById(req.params.id);
    if (!report) {
...
const getPublicReport = async (req, res) => {
  try {
    ...
    const { data: report, hit } = await getOrSetCache(cacheKey, 600, async () => {
      const row = await reportRepository.findReport({
...
const updateReport = async (req, res) => {
  try {
    const [updated] = await reportRepository.updateReports(req.body, {
...
const deleteReport = async (req, res) => {
  try {
    const deleted = await reportRepository.destroyReport({
      where: { id: req.params.id },
    });
    if (!deleted) {
      return res.status(404).json({ message: 'Weekly report not found' });
...
const getKocDetail = async (req, res) => {
  try {
    ...
    const creator = await reportRepository.findUser({ where: { id: creatorId, role: 'koc' }, attributes: ['id', 'name', 'email', 'role'], raw: true });
...
const generateWeeklyReport = async (req, res) => {
  try {
    ...
    const bookingInstances = await reportRepository.findBookingsWithVideos({
      where: {
